...
exports.createDJ = asyncHandler(async (req, res, next) => {
...
// @access   Public
exports.getDJs = asyncHandler(async (req, res, next) => {
...
// @route   POST/api/v1/songs/
// @access   Public
exports.likeDJ = asyncHandler(async (req, res, next) => {
  const user = await User.findById(req.user.id);
  const orin = await DJ.findById(req.params.id);
  if (!user) {
    return next(new ErrorResponse("Not Found", 404));
  }
  const songs = user.likedDJ;
  ...
  const a = await DJ.findByIdAndUpdate(
    orin._id,
    {
      likes: orins,
    },
...
// @access   Public
exports.likedDJs = asyncHandler(async (req, res, next) => {
  const user = await User.findById(req.user.id);
  res.status(200).json({
    success: true,
    data: user.likedDJ,
...
exports.updatePlay = asyncHandler(async (req, res, next) => {
  const song = await DJ.findById(req.params.id);
